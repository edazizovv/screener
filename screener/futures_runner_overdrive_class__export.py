...
over_mask = ~data['y'].isna() * (numpy.array(range(data.shape[0])) >= window) * (numpy.array(range(data.shape[0])) < (data.shape[0] - foresearch_limit))

thresh = int(data.loc[over_mask, :].values.shape[0] * 0.5)
thresh_ix = data.index[over_mask].values[thresh]
...
over_train = over_mask * ix_train
over_test = over_mask * ix_test

x_train = data.loc[over_train, x_features].values
y_train = data.loc[over_train, 'y'].values.astype(dtype=int)

x_test = data.loc[over_test, x_features].values
y_test = data.loc[over_test, 'y'].values.astype(dtype=int)
...
data.loc[over_train, 'filter'] = y_hat_train
data.loc[over_test, 'filter'] = y_hat_test
...
perf_train = perf.loc[over_train, :].copy()
perf_test = perf.loc[over_test, :].copy()
